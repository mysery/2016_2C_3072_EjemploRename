...
namespace TGC.Group.Physics
{
    public class WallBullet : PhysicsGame
    {
        ...
        public override void Init(GameModel ctx)
        {
            ...
            var texture = TgcTexture.createTexture(D3DDevice.Instance.Device, Ctx.MediaDir + "\\Texturas\\box1.jpg");
            ...
            boxMesh1 = TgcBox.fromSize(new Vector(20, 20, 20).ToDxVector, texture);
            texture = TgcTexture.createTexture(D3DDevice.Instance.Device, Ctx.MediaDir + "\\Texturas\\box4.jpg");
            boxMesh2 = TgcBox.fromSize(new Vector(20, 20, 20).ToDxVector, texture);
            texture = TgcTexture.createTexture(D3DDevice.Instance.Device, Ctx.MediaDir + "\\Texturas\\box3.jpg");
            boxMesh3 = TgcBox.fromSize(new Vector(20, 20, 20).ToDxVector, texture);
            texture = TgcTexture.createTexture(D3DDevice.Instance.Device, Ctx.MediaDir + "\\Texturas\\box4.jpg");
            boxMesh4 = TgcBox.fromSize(new Vector(20, 20, 20).ToDxVector, texture);
        ...
        public override void Render()
        {
            ...
            for (var i=0; i< count; i++)
            {
                ...
                if (i % 4 == 1)
                {
                    ...
                    boxMesh1.Transform = new Matrix(boxBody.InterpolationWorldTransform).ToDxMatrix;
                ...
                {
                    //Obtenemos la matrix de directx con la transformacion que corresponde a la caja.
                    boxMesh2.Transform = new Matrix(boxBody.InterpolationWorldTransform).ToDxMatrix;
                    //Dibujar las cajas en pantalla
                    boxMesh2.render();
                }
                if (i % 4 == 3)
                {
                    //Obtenemos la matrix de directx con la transformacion que corresponde a la caja.
                    boxMesh3.Transform = new Matrix(boxBody.InterpolationWorldTransform).ToDxMatrix;
                    //Dibujar las cajas en pantalla
                    boxMesh3.render();
                }
                if (i % 4 == 0)
                {
                    //Obtenemos la matrix de directx con la transformacion que corresponde a la caja.
                    boxMesh4.Transform = new Matrix(boxBody.InterpolationWorldTransform).ToDxMatrix;
                    //Dibujar las cajas en pantalla
                    boxMesh4.render();
                }
            }
